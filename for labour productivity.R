...
emu_sub <- emu[emu$Industry.code %in% c("20", "21t22","25","27t28",
                                        "34t35","E", "F", "G", "H", "60t63", "64", "J", "71t74", "L",
                                        "M", "N"),]

#PLotting
ggplot(emu_sub,aes(x=year,y=LP,color=Industry.code)) +
  geom_line() +
  geom_vline(xintercept = 2019, linetype="dashed") +
  scale_color_manual(values = c("red", "blue", "orange", "purple", "brown", 
...
plot(x = emu[emu$Industry.code == "27t28",]$year,
     y = emu[emu$Industry.code == "27t28",]$LP,
     type = "l",
     xlab = "Year",
     ylab = "Labour Productivity of Basic Metals",
     col = "red",
     lwd = 3,
     ylim = c(0,800))
     xlim = c(2004,2022)
...
lines(x = emu[emu$Industry.code == "20",]$year,
      y = emu[emu$Industry.code == "20",]$LP,
      col = "orange")

lines(x = emu[emu$Industry.code == "21t22",]$year,
      y = emu[emu$Industry.code == "21t22",]$LP,
      col = "blue")

lines(x = emu[emu$Industry.code == "25",]$year,
      y = emu[emu$Industry.code == "25",]$LP,
      col = "#00008B")
...
plot_trends(emu_synth) +
  labs(x="Year", y = "Labour Productivity", subtitle = "Basic Metals") +
...
plot_trends(emu_synth_bs)  +
  labs(x="Year", y = "Labour Productivity of Business Services",subtitle = "Business Services") +
   scale_y_continuous(labels=comma)
...
plot_trends(emu_synth) +
  labs(x="Year", y = "Labour Productivity", subtitle = "Basic Metals") +
  scale_y_continuous(labels=comma) +
  annotate("text", x = 2010, y = max(grab_synthetic_control(emu_synth)$synth_y, na.rm = TRUE) * 0.6, 
           label = paste("RMSE Ratio (Post/Pre):", round(RMSE_Basic, 3)), color = "red", size = 3)


# Plotting the Synthetic Control Trends for Basic Metals with RMSE
plot_trends(emu_synth_bs) +
  labs(x="Year", y = "Labour Productivity", subtitle = "Business Services") +
  scale_y_continuous(labels=comma) +
  annotate("text", x = 2010, y = max(grab_synthetic_control(emu_synth)$synth_y, na.rm = TRUE) * 0.6, 
